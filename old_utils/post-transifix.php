...
while($file = readdir($dh)) {
	...
	if(!is_dir($root.'/component/'.$file)) continue;
...
while($file = readdir($dh)) {
	if(substr($file,0,1) == '.') continue;
	if(!is_dir($root.'/modules/'.$file)) continue;
	
	$dh2 = opendir($root.'/modules/'. $file);
	while($subfolder = readdir($dh2)) {
		if(substr($subfolder,0,1) == '.') continue;
		if(!is_dir($root.'/modules/'. $file.'/'.$subfolder)) continue;
...
while($file = readdir($dh)) {
	...
	while($subfolder = readdir($dh2)) {
		if(substr($subfolder,0,1) == '.') continue;
		if(!is_dir($root.'/plugins/'. $file.'/'.$subfolder)) continue;
